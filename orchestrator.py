    ...
    def run_iqss(self, annees: List[int]) -> None:
        ...
        for annee in annees:
            try:
                ...
                candidates = (
                    glob.glob(os.path.join(year_folder, "*.csv")) +
                    glob.glob(os.path.join(year_folder, "*.xlsx"))
                )
                
                if not candidates:
                    raise FileNotFoundError(
                        f"No IQSS file found in {year_folder}. "
                ...
                clean_iqss(
                    filepath=filepath,
                    annee=annee,
                    silver_dir=os.path.join(self.silver, "iqss")
                )
        ...
        if failed_years:
            logger.warning(
                f"IQSS pipeline completed with failures for years: {failed_years}"
            )
