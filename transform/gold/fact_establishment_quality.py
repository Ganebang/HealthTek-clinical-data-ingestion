...
def create_fact_establishment_quality(
    ...
    for iqss_file in iqss_files:
        try:
            ...
            if iqss_file.suffix == '.csv':
                ...
                try:
                    df_year = pd.read_csv(iqss_file, sep=';', encoding='utf-8', dtype={'finess': str})
                except UnicodeDecodeError:
                    df_year = pd.read_csv(iqss_file, sep=';', encoding='latin1', dtype={'finess': str})
            else:
    ...
    score_cols = [c for c in df_iqss.columns if 'score' in c.lower() and 'all' in c.lower() and 'ajust' in c.lower()]
    ...
    if df_iqss['score_all_ssr_ajust'].isna().all():
         ...
         if fallback:
             df_iqss['score_all_ssr_ajust'] = df_iqss['score_all_ssr_ajust'].fillna(df_iqss[fallback])
         else:
             # Only warn if we really still have nothing
             if df_iqss['score_all_ssr_ajust'].isna().all():
                 logger.warning("Could not find overall score column. Evolution metrics will be empty.")
