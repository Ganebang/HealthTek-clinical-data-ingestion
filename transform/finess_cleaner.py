...
def clean_finess(filepath: str, silver_dir: Optional[str] = None) -> pd.DataFrame:
    """
    ...
    try:
        ...
        if df is None:
            raise FinessTransformError(
                "Failed to decode file with supported encodings (utf-8, latin1, cp1252)"
            )
    ...
    try:
        ...
        for col in text_cols:
            if col in df.columns:
                df[col] = (
                    df[col].astype(str).str.strip().str.title().str.replace("_", " ", regex=False)
                )
